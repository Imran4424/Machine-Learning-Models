...
![Support Vector Regression](Images/SupportVector/SupportVectorRegression.png)

This tube has a width of $\epsilon$ and the width is measured vertically along the axis not perpendicular to the tube but vertically. This tube itself is called the epsilon-insensitive tube. Any point in our dataset that falls inside the tube won't be disregarded as an error. This epsilon-insensitive tube works as a margin of error that we are allowing our model to have and does not care about any error inside here. Compared to the Linear Regression model, the Support Vector Regression model provides a bit of buffer through the epsilon-insensitive tube.
...
![Support Vectors](Images/SupportVector/SupportVectorRegression2.png)
...
We can build a Random Forest regression model using the following steps
...
The Mathematical formula for the Random Forest Model,
...
![Support Vectors](Images/SupportVector/MaximumMarginHyperplane.png)
...
Kernel Support Vector Machine is special type of Support Vector machine model which can separable linearly non
...
Decision Tree Classification is a non-linear classification model that can handle complex datasets with complicated patterns. It classifies the dataset using a tree-like model, making it flexible and easy to interpret.

Decision Tree Classifier at the core is a binary tree that classifies a given dataset. This model creates a root node on the entire dataset and then splits the data into smaller subsets based on decision rules derived from the input features. Each split is made to minimize the error in classifying the target dataset.
...
$$
\boldsymbol{\hat{y} = \arg \max_c \displaystyle\sum_{i=1}^{T}1(f_i(x) = c)}
$$
